...
def model_opts(parser):
    ...
    group.add_argument('-tgt_word_vec_size', type=int, default=400,
                       help='Word embedding size for tgt.')
    group.add_argument('-word_vec_size', type=int, default=150,
    ...
    group.add_argument('-item_embedding', type=str, default=None,
                       choices=['none', 'name', 'comment', 'both'],
                       help="""Add pretrained item symantic embedding.""")
    group.add_argument('-loc_embedding', action="store_true",
                       help="""Add pretrained location embedding.""")

    group.add_argument('-dwelltime_attn', action="store_true",
                       help="""Add dwelltime attention.""")

    # Encoder-Decoder Options
    group = parser.add_argument_group('Model- Encoder-Decoder')
    group.add_argument('-only_encoder', action="store_true",
                       help="""Evaluate the validity of transformer.""")

    group.add_argument('-no_predictor', action="store_true",
                       help="""Evaluate the validity of recommendation.""")
    
    group.add_argument('-ablation', type=str, default=None,
                       help="""Ablation Experinment.""")
    ...
    group.add_argument('-decoder_type', type=str, default='transformer',
                       ...
                       are experimental. Options are
                       [rnn|transformer|cnn].""")
    group.add_argument('-generator_position', type=str, default='encoder',
                       choices=['encoder', 'decoder'],
                       help="""Choose the postion of generator.
                       encoder: the generator is after encoder.
                       decoder: the generator is after decoder.""")

    group.add_argument('-layers', type=int, default=1,
                       help='Number of layers in enc/dec.')
    group.add_argument('-enc_layers', type=int, default=8,
                       help='Number of layers in the encoder')
    group.add_argument('-dec_layers', type=int, default=4,
                       help='Number of layers in the decoder')
    group.add_argument('-rnn_size', type=int, default=150,
                       help="""Size of rnn hidden states. Overwrites
                       enc_rnn_size and dec_rnn_size""")
    group.add_argument('-enc_rnn_size', type=int, default=400,
                       help="""Size of encoder rnn hidden states.
                       Must be equal to dec_rnn_size except for
                       speech-to-text.""")
    group.add_argument('-dec_rnn_size', type=int, default=400,
    ...
    group.add_argument('-heads', type=int, default=2,
                       help='Number of heads for transformer self-attention')
    group.add_argument('-transformer_ff', type=int, default=128,
...
def preprocess_opts(parser):
    ...
    group.add_argument('-train_src', default="data/src-train.txt",
                       help="Path to the training source data")
    group.add_argument('-train_tgt', default="data/tgt-train.txt",
                       help="Path to the training target data")
    group.add_argument('-valid_src', default="data/src-valid.txt",
                       help="Path to the validation source data")
    group.add_argument('-valid_tgt', default="data/tgt-valid.txt",
                       help="Path to the validation target data")
    group.add_argument('-topk_keywords', default=20, type=int)

    group.add_argument('-save_data', default="data/data",
                       help="Output file for the prepared data")

    group.add_argument('-max_shard_size', type=int, default=0,
                       help="""Deprecated use shard_size instead""")

    group.add_argument('-shard_size', type=int, default=100000,
    ...
    group.add_argument('-src_vocab_size', type=int, default=None,
    ...
    group.add_argument('-dynamic_dict', type=bool, default=False,
    ...
    group.add_argument('-src_seq_length', type=int, default=20,
                       help="Maximum source sequence length (Session sequence length)")
    ...
    group.add_argument('-log_file', type=str, default="logs/preprocess.log",
...
def train_opts(parser):
    ...
    group.add_argument('-data', default='data/data',
                       help="""Path prefix to the ".train.pt" and
                       ".valid.pt" file path from preprocess.py""")

    group.add_argument('-save_model', default='models/trans',
    ...
    group.add_argument('-valid_steps', type=int, default=5000,
    ...
    group.add_argument('-max_generator_batches', type=int, default=1000,
                       help="""Maximum batches of words in a sequence to run
                        ...
                        uses more memory.""")
    group.add_argument('-train_steps', type=int, default=60000,
                       help='Number of training steps')
    group.add_argument('-epochs', type=int, default=0,
                       help='Deprecated epochs see train_steps')
    group.add_argument('-optim_e', default='adam',
                       choices=['sgd', 'adagrad', 'adadelta', 'adam',
                                'sparseadam'],
                       help="""Optimization method.""")
    group.add_argument('-optim_d', default='adam',
                       choices=['sgd', 'adagrad', 'adadelta', 'adam',
                                'sparseadam'],
                       help="""Optimization method.""")
    group.add_argument('-adagrad_accumulator_init_e', type=float, default=0,
                       help="""Initializes the accumulator values in adagrad.
                       Mirrors the initial_accumulator_value option
                       in the tensorflow adagrad (use 0.1 for their default).
                       """)
    group.add_argument('-loss_weight_e', type=float, default=0.7,
                       help="""The loss weight of click prediction.
                       The loss weight of keyphrase generation is 1 - loss_weight_e""")

    group.add_argument('-adagrad_accumulator_init_d', type=float, default=0,
                       help="""Initializes the accumulator values in adagrad.
                       Mirrors the initial_accumulator_value option
                       in the tensorflow adagrad (use 0.1 for their default).
                       """)
    group.add_argument('-max_grad_norm_e', type=float, default=5,
                       help="""If the norm of the gradient vector exceeds this,
                       renormalize it to have the norm equal to
                       max_grad_norm""")
    group.add_argument('-max_grad_norm_d', type=float, default=5,
                       help="""If the norm of the gradient vector exceeds this,
                       renormalize it to have the norm equal to
                       max_grad_norm""")
    ...
    group.add_argument('-truncated_decoder', type=int, default=0,
                       help="""Truncated bptt.""")
    group.add_argument('-adam_beta1_e', type=float, default=0.9,
                       help="""The beta1 parameter used by Adam.
                       Almost without exception a value of 0.9 is used in
                       the literature, seemingly giving good results,
                       so we would discourage changing this value from
                       the default without due consideration.""")
    group.add_argument('-adam_beta1_d', type=float, default=0.9,
                       help="""The beta1 parameter used by Adam.
                       Almost without exception a value of 0.9 is used in
                       the literature, seemingly giving good results,
                       so we would discourage changing this value from
                       the default without due consideration.""")
    group.add_argument('-adam_beta2_e', type=float, default=0.999,
                       ...
                       baselines.""")
    group.add_argument('-adam_beta2_d', type=float, default=0.999,
                       help="""The beta2 parameter used by Adam.
                       Typically a value of 0.999 is recommended, as this is
                       the value suggested by the original paper describing
                       Adam, and is also the value adopted in other frameworks
    ...
    group.add_argument('-learning_rate_e', type=float, default=0.001,
                       help="""Starting learning rate.
                       Recommended settings: sgd = 1, adagrad = 0.1,
                       adadelta = 1, adam = 0.001""")
    group.add_argument('-learning_rate_d', type=float, default=0.001,
                       help="""Starting learning rate.
                       Recommended settings: sgd = 1, adagrad = 0.1,
                       adadelta = 1, adam = 0.001""")
    group.add_argument('-learning_rate_decay_e', type=float, default=0.5,
                       help="""If update_learning_rate, decay learning rate by
                       this much if (i) perplexity does not decrease on the
                       validation set or (ii) steps have gone past
                       start_decay_steps""")
    group.add_argument('-learning_rate_decay_d', type=float, default=0.5,
                       help="""If update_learning_rate, decay learning rate by
                       this much if (i) perplexity does not decrease on the
                       validation set or (ii) steps have gone past
                       start_decay_steps""")
    group.add_argument('-start_decay_steps_e', type=int, default=30000,
                       help="""Start decaying every decay_steps after
                       start_decay_steps""")
    group.add_argument('-start_decay_steps_d', type=int, default=30000,
                       help="""Start decaying every decay_steps after
                       start_decay_steps""")
    group.add_argument('-decay_steps_e', type=int, default=10000,
                       help="""Decay every decay_steps""")
    group.add_argument('-decay_steps_d', type=int, default=10000,
                       help="""Decay every decay_steps""")

    group.add_argument('-decay_method_e', type=str, default=None,
                       choices=['noam'], help="Use a custom decay rate.")
    group.add_argument('-decay_method_d', type=str, default=None,
                       choices=['noam'], help="Use a custom decay rate.")
    ...
    group.add_argument('-log_file', type=str, default="logs/train.log",
                       help="Output logs to a file under this path.")
...
def translate_opts(parser):
    ...
    group.add_argument('-src', default='data/src-test.txt',
                       help="""Source sequence to decode (one line per
                       sequence)""")
    group.add_argument('-tgt', default='data/tgt-test.txt',
                       help='True target sequence (optional)')
    group.add_argument('-output', default='data/pred.txt',
    ...
    group.add_argument('-input_weight', default=10, type=int)
    group.add_argument('-last_weight', default=15, type=int)
    group.add_argument('-skip_keyphrase', default=False, type=bool)
    ...
    group.add_argument('-log_file', type=str, default="logs/translate.log",
                       help="Output logs to a file under this path.")
