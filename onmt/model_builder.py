...
def build_encoder(opt, embeddings):
    ...
    if opt.encoder_type == "transformer":
        return TransformerEncoder(opt.enc_layers, opt.enc_rnn_size,
                                  ...
                                  opt.dropout, embeddings, ablation=opt.ablation)
...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    # Build encoder.
    src_dict = fields["src_sku"].vocab
    feature_dicts = inputters.collect_feature_vocabs(fields, 'src_sku')
    src_embeddings = build_embeddings(
        model_opt, src_dict, feature_dicts, side='src_sku')
    ...
    if model_opt.only_encoder:
        # Build NMTModel(= encoder).
        decoder = None
        logger.info("Build NMT Model only encoder.")
    ...
    if not model_opt.copy_attn:
        ...
        if model_opt.share_decoder_embeddings:
            ...
            predictor[0].weight = decoder.embeddings.word_lut.weight
