...
def collect_feature_vocabs(fields, side):
    ...
    """
    assert side in ['src', 'tgt', 'src_sku', 'tgt_sku']
    feature_vocabs = []
    for j in count():
        key = side + "_feat_" + str(j)
        if key not in fields:
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    if data_type == 'text':
        ...
        # Share SKU Vocab
        for _f in ["sku"]:
            logger.info(" * merging src_{} and tgt_{} vocab...".format(_f, _f))
            merged_vocab = merge_vocabs(
                [fields["src_"+_f].vocab, fields["tgt_"+_f].vocab])
            fields["src_"+_f].vocab = merged_vocab
            fields["tgt_"+_f].vocab = merged_vocab
...
def _load_fields(dataset, data_type, opt, checkpoint):
    ...
    logger.info(' * vocabulary size. source = %d; target = %d' %
                (len(fields['src_sku'].vocab), len(fields['tgt'].vocab)))
