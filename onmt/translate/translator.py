...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    translator = Translator(model, fields, global_scorer=scorer,
                            ...
                            input_weight=opt.input_weight, last_weight=opt.last_weight,
                            topk_keywords=opt.topk_keywords, skip_keyphrase=opt.skip_keyphrase,
                            pred_save_path = opt.models[0],
                            **kwargs)
    return translator
...
class Translator(object):
    ...
    def __init__(self,
        ...
        self.skip_keyphrase = skip_keyphrase
    ...
    def translate(self,
        ...
        for batch in data_iter:
            batch_data = self.translate_batch(batch, data, fast=self.fast, skip_keyphrase=self.skip_keyphrase)

            # for click prediction
            scores_sku = batch_data["score_sku_prob"]
            # [b X 20]
            sorted_index_t = np.argsort(-scores_sku.detach())[:, :20].to(scores_sku.device)
            ...
            pred_sents_list.extend([' '.join(_tran.pred_sents[0]) for _tran in translations])
            gold_sents_list.extend([' '.join(_tran.gold_sent) for _tran in translations])
        ...
        self.logger.info('Recall@1: {}\t\tMrr@1: {}'.format(
            np.array(_recall_1_result).mean(), np.array(_mrr_1_result).mean()))
        self.logger.info('Recall@5: {}\t\tMrr@5: {}'.format(
            np.array(_recall_5_result).mean(), np.array(_mrr_5_result).mean()))
        self.logger.info('Recall@10: {}\t\tMrr@10: {}'.format(
            np.array(_recall_10_result).mean(), np.array(_mrr_10_result).mean()))
        self.logger.info('Recall@15: {}\t\tMrr@15: {}'.format(
            np.array(_recall_15_result).mean(), np.array(_mrr_15_result).mean()))
        self.logger.info('Recall@20: {}\t\tMrr@20: {}'.format(
            np.array(_recall_20_result).mean(), np.array(_mrr_20_result).mean()))

        def save_pred(_results_dirname, _name, _results, _cut_off ):
            with open('{}/pred_{}@{}'.format(_results_dirname, _name, _cut_off), 'w') as f:
        ...
        if len(gold_sents_list) > 0:
            rouge = Rouge()
            rouge_scores = rouge.get_scores(pred_sents_list, gold_sents_list, avg=True)
            self.logger.info('Rouge-1:\tP:{}\t\tR:{}\t\tF1:{}'.format(rouge_scores['rouge-1']['p'], rouge_scores['rouge-1']['r'], rouge_scores['rouge-1']['f']))
            self.logger.info('Rouge-2:\tP:{}\t\tR:{}\t\tF1:{}'.format(rouge_scores['rouge-2']['p'], rouge_scores['rouge-2']['r'], rouge_scores['rouge-2']['f']))
            self.logger.info('Rouge-L:\tP:{}\t\tR:{}\t\tF1:{}'.format(rouge_scores['rouge-l']['p'], rouge_scores['rouge-l']['r'], rouge_scores['rouge-l']['f']))
    ...
    def translate_batch(self, batch, data, fast=False, skip_keyphrase=False):
        """
        ...
        with torch.no_grad():
            if fast:
                ...
                return self._translate_batch(batch, data, skip_keyphrase=skip_keyphrase)

    def _run_encoder(self, batch, data_type):
        src_sku = inputters.make_features(batch, 'src_sku', data_type)
        _, src_sku_lengths = batch.src_sku
        enc_states, memory_bank, src_sku_lengths, encoded = self.model.encoder(
            src_sku, src_sku_lengths, input_weight=self.input_weight, last_weight=self.last_weight)
        if src_sku_lengths is None:
            assert not isinstance(memory_bank, tuple), \
                'Ensemble decoding only supported for text data'
            src_sku_lengths = torch.Tensor(batch.batch_size) \
                               .type_as(memory_bank) \
                               .long() \
                               .fill_(memory_bank.size(0))
        return src_sku, enc_states, memory_bank, src_sku_lengths, encoded
    ...
    def _fast_translate_batch(self,
        ...
        # Encoder forward.
        src_sku, enc_states, memory_bank, src_sku_lengths, encoded = self._run_encoder(
            batch, data.data_type)
        dec_states = self.model.decoder.init_decoder_state(
            src_sku, memory_bank, enc_states, with_cache=True)

        # Tile states and memory beam_size times.
        dec_states.map_batch_fn(
            lambda state, dim: tile(state, beam_size, dim=dim))
        memory_bank = tile(memory_bank, beam_size, dim=1)
        memory_lengths = tile(src_sku_lengths, beam_size)
    ...
    def _translate_batch(self, batch, data, skip_keyphrase=False):
        ...
        # (1) Run the encoder on the src_sku.
        src_sku, enc_states, memory_bank, src_sku_lengths, encoded = self._run_encoder(
            batch, data_type)

        # (1-2) Predict target item.
        predicted_sku = self.model.predictor.forward(encoded.view(-1, encoded.size(2))).data
        if self.model.decoder is None or skip_keyphrase:
            return {"score_sku_prob": predicted_sku, "batch": batch, "gold_score": [0] * batch_size, "attention": [[] for _ in range(batch_size)], "scores": [[] for _ in range(batch_size)], "predictions": [[] for _ in range(batch_size)]}

        dec_states = self.model.decoder.init_decoder_state(
            src_sku, memory_bank, enc_states)

        # (2) Repeat src_sku objects `beam_size` times.
        ...
        memory_lengths = src_sku_lengths.repeat(beam_size)
        ...
        for i in range(self.max_length):
            ...
            _outputs_list.append(unbottle(dec_out))
        ...
        ret["batch"] = batch
        ret["score_sku_prob"] = predicted_sku
        return ret
    ...
    def _from_beam(self, beam):
        ...
        for b in beam:
            ...
            for i, (times, k) in enumerate(ks[:n_best]):
                hyp, att, path = b.get_hyp(times, k)
                hyps.append(hyp)
                attn.append(att)
                paths.append(path)
            ret["predictions"].append(hyps)
            ret["scores"].append(scores)
            ret["beam_search_path"].append(paths)
    ...
    def _run_target(self, batch, data):
        ...
        #  (1) run the encoder on the src_sku
        enc_states, memory_bank, src_sku_lengths, _ \
            = self.model.encoder(src_sku, src_sku_lengths)
        dec_states = \
            self.model.decoder.init_decoder_state(src_sku, memory_bank, enc_states)
        ...
        dec_out, _, _ = self.model.decoder(
            tgt_in, memory_bank, dec_states, memory_lengths=src_sku_lengths)
