...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, dropout, dim, max_len=100000):
...
class Embeddings(nn.Module):
    ...
    def __init__(self, word_vec_size,
        ...
        if feat_merge == 'mlp' and len(feat_vocab_sizes) > 0:
            ...
            mlp = nn.Sequential(nn.Linear(in_dim, out_dim), nn.LeakyReLU())
    ...
    def load_pretrained_vectors_from_tensor(self, pretrained, fixed):
        """Load in pretrained embeddings.

        Args:
          pretrained (tensor) : torch serialized embeddings
          fixed (bool) : if true, embeddings are not updated
        """
        pretrained_vec_size = pretrained.size(1)
        if self.word_vec_size > pretrained_vec_size:
